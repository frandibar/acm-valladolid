/* @JUDGE_ID:  40922FK  596  C++  */
/* @BEGIN_OF_SOURCE_CODE */    
/**************************************************************************
 Solution to Problem 596 - The Incredible Hull
 by: Francisco Dibar
 date: Nov-29-2005
**************************************************************************/
...
int main(int argc, char *argv[])
{
	...
	is >> header;
	while ((header != END) && (!cin.eof())) {
		int k = 0;
		// READ INPUT
		vector<point> vec_points;
		if (header == SAMPLE) {
			is >> header >> k;		// ignore header
			cout << "Sample " << k << " convex hull:" << endl;
			cout << " ";

			// READ NEXT INPUT HEADER
			getline(cin, line);
			is.clear();
			is.str(line);
			is >> header;
		...
		} else while (header == POLYGON) {			
			int npoints = 0;
			is >> npoints;
			vec_points.resize(vec_points.size() + npoints);
			for (int i = npoints; i > 0; i--)
				is >> vec_points[vec_points.size()-i].x() >> vec_points[vec_points.size()-i].y();
		...
		// SHOW OUTPUT
		// start by point most to the right, and to the top, find it
		double right, top;
		right = hull.vec_points[0].x();
		top = hull.vec_points[0].y();
		int s = 0;	// starting index
		for (int j = 1; j < hull.get_n(); j++) {
			if (hull.vec_points[j].x() > right) {
				s = j;
				right = hull.vec_points[j].x();
				top = hull.vec_points[j].y();
			} else if (hull.vec_points[j].x() == right) {
				if (hull.vec_points[j].y() < top) {
					s = j;
					top = hull.vec_points[j].y();
				}
			}
		...
		// REVISAR!! debo agregar los puntos de las figuras colineales con el hull
		int npts = hull.get_n();
		for (int i = 0; i < npts-1; i++) {
			int ind = (i + s) % npts;
			cout << hull.vec_points[ind] << " ";
		}
		cout << hull.vec_points[(npts-1 + s) % npts] << endl;
	}
