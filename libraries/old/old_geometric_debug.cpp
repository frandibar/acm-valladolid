/* @JUDGE_ID:  40922FK  10135  C++  */
/* @BEGIN_OF_SOURCE_CODE */    
/**************************************************************************
 Solution to Problem 10135 - Herding Frosh
 by: Francisco Dibar
 date: Nov-26-2005
**************************************************************************/
...
//	inline double& x() { return (_dim > 0 ? _vec_coords[0] : 0); }	// error
	...
	inline double get_coord(unsigned int i) { return (_dim > i ? _vec_coords[i] : 0); }
...
// returns int instead of bool to be able to use as sorting order
// valid for 2D only!!
{
...
//				if (first_point.square_distance(vec_points[hole]) < first_point.square_distance(vec_points[i]))
				double dh = first_point.square_distance(vec_points[hole]);
				double di = first_point.square_distance(vec_points[i]);
				if (dh < di)
...
/*	//	quitar
		cout << "after removing same angle" << endl;
		for (int j = 0; j < _n; j++)
			cout << vec_points[j] << endl;
*/
	}
...
/*	//	quitar
		...
		for (int j = 0; j < _n; j++)
			cout << vec_points[j] << endl;
*/
		//quicksort_poly(vec_points, 1, _n-1, first_point);
		selection_sort_poly(first_point);
		//insertion_sort_poly(first_point);
/*	// quitar
		cout << "after" << endl;
		for (int j = 0; j < _n; j++)
			cout << vec_points[j] << endl;
...
/*	// quitar
		cout << "points in order leftlower" << endl;	
		for (int j = 0; j < _n; j++)
			cout << vec_points[j] << endl;
*/
		//sort(vec_points.begin(), vec_points.end(), less_angle_cmp(first_point));
		sort_points_by_angle(first_point);
/*	// quitar
		cout << "points in order by angle" << endl;	
		for (int j = 0; j < _n; j++)
			cout << vec_points[j] << endl;
*/
...
/////////////////////////////////////////////////////////////////////////// 
double calc_silk(vector<point>& vec_points)
{
	...
	for (int i = 0; i < p.get_n(); i++)
		cout << p.vec_points[i] << endl;
/*
	cout << "polygon sorted by leftlower" << endl;
	sort(p.vec_points.begin(), p.vec_points.end()-1, leftlower);
	for (int i = 0; i < p.get_n(); i++)
		cout << p.vec_points[i] << endl;

	// test sort_and_remove_duplicate_points
	cout << "polygon without duplicate points" << endl;
	p.sort_and_remove_duplicate_points();
	for (int i = 0; i < p.get_n(); i++)
		cout << p.vec_points[i] << endl;
*/
	// test convex hull
	polygon h = p.convex_hull();
	cout << "convex hull" << endl;
	for (int i = 0; i < h.get_n(); i++)
		cout << h.vec_points[i] << endl;
...
int main(int argc, char *argv[])
{
	...
	for (int k = 0; k < cases; k++) {
		...
		// PROCESS
		double silk = calc_silk(vec_points);
