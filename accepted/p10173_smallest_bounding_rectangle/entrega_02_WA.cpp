...
int main(int argc, char *argv[])
{
	...
	while ((points != 0) && !cin.eof()) {
		...
		// PROCESS
		polygon hull = polygon(vec_points).convex_hull();
#ifdef DEBUG
		cout << START_DEBUG;
		cout << "hull " << endl << hull << endl;
#endif
		
		// find longest side, translate to 0,0 and rotate along its side.
		double longest = 0;
		int index = 0;
		for (int i = 0; i < hull.get_n(); i++) {
			double dist = hull.vec_points[i].square_distance(hull.vec_points[(i+1) % hull.get_n()]);
			if (dist > longest) {
				longest = dist;
				index = i;
			}
		}
		hull.translate(-hull.vec_points[index]);
		point<2> next = hull.vec_points[(index+1) % hull.get_n()];
		double alfa = 0;
#ifdef DEBUG
		...
		if (std::fabs(next.square_modulus()) > EPSILON) {
			...
			hull.rotate(-alfa);
#ifdef DEBUG
		cout << "hull after rotating " << -alfa << " " << endl << hull << endl;
		cout << END_DEBUG;
#endif
		}
