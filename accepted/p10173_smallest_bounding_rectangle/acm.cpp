...
///////////////////////////////////////////////////////////////////////////
double poly_in_rectangle_area(const polygon& poly)
{
#ifdef DEBUG
	cout << START_DEBUG;
	cout << "hull " << endl << poly << endl;
#endif

	// for each side side, translate to 0,0 and rotate along its side.
	double minarea = -1;
	for (int i = 0; i < poly.get_n(); i++) {
		// use new hull to translate & rotate, avoids precision errors
		polygon hull = poly;	
		hull.translate(-hull.vec_points[i]);
#ifdef DEBUG
		cout << "hull after translating " << endl << hull << endl;
#endif
		point<2> next = hull.vec_points[(i+1) % hull.get_n()];
		double alfa = 0;
		if (std::fabs(next.square_modulus()) > EPSILON) {
			alfa = asin(next.y() / next.modulus());
			// determine cuadrant
			// now alfa is valid if it is in the I or IV cuadrant
			if (next.y() >= 0) {		// alfa >= 0
				// I or II cuadrant
				if (next.x() < 0)
					// II cuadrant
			...
			hull.rotate(-alfa);
#ifdef DEBUG
		cout << "hull after rotating " << -alfa << " (" << -alfa*180/PI << ")" << endl << hull << endl;
#endif
		}
		double xmax, xmin, ymax, ymin;
		// calculate extreme points
		xmin = xmax = hull.vec_points[0].x();
		ymin = ymax = hull.vec_points[0].y();
 		for (int i = 1; i < hull.get_n(); i++) {
			xmin = std::min(xmin, hull.vec_points[i].x());
			xmax = std::max(xmax, hull.vec_points[i].x());
			ymin = std::min(ymin, hull.vec_points[i].y());
			ymax = std::max(ymax, hull.vec_points[i].y());
		}
...
int main(int argc, char *argv[])
{
	...
	while ((points != 0) && !cin.eof()) {
		// READ INPUT
		vector< point<2> > vec_points(points);
		for (int i = 0; i < points; i++)
			cin >> vec_points[i].x() >> vec_points[i].y();

		// PROCESS
		polygon hull = polygon(vec_points).convex_hull(false);
		double minarea = poly_in_rectangle_area(hull);
