...
////////////////////////////////////////////////////////////////////////////////
void progreso(vector<vector<char> >& mat_maze, int row, int pila)
{
	cout << "pila: " << pila << endl << endl;
	for (int i = 0; i < row; i++) {
		string str_line = "";
		int largo = 0;
...
// xi: fila inicial, yi: col inicial
{
	...
	while (!que_faltan.empty()) {
		...
		#ifdef DEBUG_FD
			progreso(mat_maze, mat_maze.size(), que_faltan.size());
		#endif 
	}
}

////////////////////////////////////////////////////////////////////////////////
void roomfill(vector<vector<char> >& mat_maze, int xi, int yi)
// xi: fila inicial, yi: col inicial
{
	queue<pair<int, int> > que_faltan;
	...
	while (!que_faltan.empty()) {
		pair<int, int> pos;
		pos = que_faltan.front();
		// pintar todo el cuarto, se asume que se esta parado en el centro del cuarto
		mat_maze[pos.first-1][pos.second-1] = PAINT;
		mat_maze[pos.first-1][pos.second  ] = PAINT;
		mat_maze[pos.first-1][pos.second+1] = PAINT;
		mat_maze[pos.first  ][pos.second-1] = PAINT;
		mat_maze[pos.first  ][pos.second  ] = PAINT;
		...
		mat_maze[pos.first+1][pos.second+1] = PAINT;
		que_faltan.pop();
		if ((mat_maze[pos.first-2][pos.second] != WALL) && 
			 (mat_maze[pos.first-2][pos.second] != SEP) && 
			 (mat_maze[pos.first-2][pos.second] != PAINT)) {
             mat_maze[pos.first-2][pos.second] = PAINT;
             que_faltan.push(make_pair(pos.first-4, pos.second));
		}
		...
		if ((mat_maze[pos.first][pos.second-2] != WALL) && 
			 (mat_maze[pos.first][pos.second-2] != SEP) && 
			 (mat_maze[pos.first][pos.second-2] != PAINT)) {
             mat_maze[pos.first][pos.second-2] = PAINT;
             que_faltan.push(make_pair(pos.first, pos.second-4));
		}
