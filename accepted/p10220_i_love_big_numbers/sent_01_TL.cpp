...
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
bigint factorial(int n, int *max, vector<bigint>& vec_fact)
// vec_fibo es un vector de la serie precalculada (max valores)
{
	if (n >= *max) {
		...
		vec_fact.resize(n+1);
		for (int i = *max; i <= n; i++)
			vec_fact[i] = (bigint)i * vec_fact[i-1];
		*max = n;
	}
	return vec_fact[n];
}
...
int main(int argc, char *argv[])
{
	...
	vector<bigint> vec_fact(2);		// valores precalculados

	vec_fact[0] = 0;
	vec_fact[1] = 1;
	cin >> n;
	while (!cin.eof()) {
		bigint fact = factorial(n, &max, vec_fact);
		cout << fact.suma_digitos() << endl;
		cin >> n;
	}
	return 0;
}
