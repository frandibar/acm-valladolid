...
public:
	...
	void actualizar() { 
		m_iLibres = m_iPieza_a | m_iPieza_b | m_iPieza_c | m_iPieza_d | m_iPieza_e | m_iPieza_f | m_iPieza_g; 
	}

	__int32 &libres() { return m_iLibres; }
	...
	void reiniciar() { 
		m_iLibres = m_iPieza_a = m_iPieza_b = m_iPieza_c = m_iPieza_d = m_iPieza_e = m_iPieza_f = m_iPieza_g = 0;
		m_sLetras = "";
	}
...
//	pos_a = "......aa.......a........a..";
	pos_a = "aa.a..a....................";	// QUITAR!!
	// Si pos_a no se encuentra en pos_pieza_a[], le aplico sucesivas rotaciones hasta hallarlo.
	// luego resuelvo el cubo para pos_pieza_a[i], y a la solucion le aplico la rotacion inversa
//	pos_a = "aa.a..a....................";	// QUITAR!!
	string pos_a_original = pos_a;
	int i, veces = 0;
	ejes eje;
	// rotar a lo largo del eje X hasta 3 veces, luego el Y y luego el Z
	while (((i = indice(pos_a)) == -1) && (veces < 9)) {
		veces++;
		pos_a = rotar(pos_a_original, eje = (ejes)int((veces-1)/3), veces);
	}
	veces = (veces - 3*eje) % 3;
	que_input.push(pos_a);					// QUITAR!!
											...
											if ((cubo.libres() & pos_pieza_f[pf]) == 0) {
												...
												for (int pg = 0; pg < POSICIONES_PIEZA_G; pg++) {
													if ((cubo.libres() & pos_pieza_g[pg]) == 0) {
														...
														cubo_rotado = rotar(cubo_rotado, eje, 4-veces);
														cout << cubo_rotado << "\t" << eje << "\t" << veces << endl;
														cubo.mostrar();
														cout << endl;
														cubo.pieza_g() = 0;
														cubo.actualizar();
