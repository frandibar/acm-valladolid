...
/////////////////////////////////////////////////////////////////////////// 
double calc_silk(const vector< point<2> >& vec_points)
{
	double perim;
	if (vec_points.size() >= 3) {
		...
		// determine if 0,0 is part of the hull
		bool in = false;
		point<2> origin(0,0);
		for (int i = 0; i < hull.get_n(); i++) {
			if (hull.vec_points[i] == origin) {
				in = true;
				break;
			}
		}
		if (!in) {
			// REVISAR!! el metodo no es correcto
			// determine 2 points in hull closest to 0,0
			// s is closer than t or equal
			int s, t;
			double ds, dt;
			ds = hull.vec_points[0].square_modulus();
			s = 0;
			dt = hull.vec_points[1].square_modulus();
			t = 1;
			if (ds < dt) {
				std::swap(ds, dt);
				std::swap(s, t);
			...
			for (int i = 2; i < hull.get_n(); i++) {
				double d = hull.vec_points[i].square_modulus();
				if (d < ds) {
					dt = ds;
					t = s;
					ds = d;
			...
			// deduce segment from s to t and and add ds and dt
			perim -= hull.vec_points[s].distance(hull.vec_points[t]);
			perim += sqrt(ds) + sqrt(dt);
		}	
	} else if (vec_points.size() == 0)
		return 2;	// never gets here
	else if (vec_points.size() == 1)
		// should never get here
		return (2 * vec_points[0].modulus() + 2);	
	else if (vec_points.size() == 2)
		return (2 * vec_points[0].modulus() + 2);
//		commented since vec_points[1] = 0,0
//		return (vec_points[0].modulus() + vec_points[1].modulus() + 
//					vec_points[0].distance(vec_points[1]) + 2);

	// add 1 meter at each end
	return (perim + 2);
}
...
int main(int argc, char *argv[])
{
	...
	for (int k = 0; k < cases; k++) {
		...
		int npoints;
		cin >> npoints;
		vector< point<2> > vec_points(npoints+1);
		for (int j = 0; j < npoints; j++) {
			cin >> vec_points[j].x() >> vec_points[j].y();
		}
		// add 0,0
		vec_points[npoints].x() = 0;
		vec_points[npoints].y() = 0;
