...
TODO: exceptions are catched but abort execution
**************************************************************************/
...
int main(int argc, char *argv[])
{
    ...
    vector<string> vPrecalculated(1, "1"); // vPrecalculated[i] = 2^i
	string n;	// read number as string
	cin >> n;
	while (!cin.eof()) {
		// PROCESS & SHOW OUTPUT
        bool found = false;
		for (int i = 0; i < (int)vPrecalculated.size(); ++i) {	// first 6 are empty
			if ((n == vPrecalculated[i].substr(0, n.length())) && (vPrecalculated[i].length() > 2 * n.length())) {
				found = true;
                cout << i << endl;
                break;
			}
		...
		if (!found) {
            // calculate more values of 2^i until found
            // there is ALWAYS a solution
            while ((n != vPrecalculated[vPrecalculated.size()-1].substr(0, n.length())) ||
                 (vPrecalculated[vPrecalculated.size()-1].length() <= 2 * n.length())) {
#ifdef DEBUG
                cout << "2^" << vPrecalculated.size()-1 << " = " << vPrecalculated[vPrecalculated.size()-1] << endl;
#endif
                Bigint temp = 2 * Bigint(vPrecalculated[vPrecalculated.size()-1]);
                vPrecalculated.push_back(temp.toString());
            }
            cout << vPrecalculated.size()-1 << endl;
        }
