/* @JUDGE_ID:  40922FK  10515  C++  */
/* @BEGIN_OF_SOURCE_CODE */    
/**************************************************************************
 Solution to Problem 10515 - Powers et al.
 by: Francisco Dibar
 date: Mar-06-2005
**************************************************************************/
...
public:
	...
	// funcion miembro creada para el problema
	// devuelvo el indice 0 porque el nro se guarda en orden inverso
	char ultimo_digito() const { return digits[0]; }	
};
...
// a^n = a^(n/2) * a^(n/2) * a^(n mod 2)
{
	...
	bigint m2 = (*this) * (*this),
			 tmp = m2,
			 //cant_m2 = nro.absolute() / 2,
			 log2 = 0,
			 n = nro;

	// hallar el log entero en base 2
	while (n >= 2) {
		n /= 2;
		log2++;
	}

	for (bigint i = 0; i < log2-1; i++) {
		tmp *= tmp;
	}


/*
	for (bigint i = 0; i < cant_m2; i++) {
		tmp *= m2;
	}
*/
	if ((nro % 2) != 0)		// si n es impar
		tmp *= (*this);
...
int main(int argc, char *argv[])
{
	...
	bigint n, m;
	cin >> n >> m;
   while (!cin.eof() && ((n != 0) || (m != 0))) {
		bigint res = n^m;
		cout << res << "  " << endl;
//		cout << res.ultimo_digito() << endl;
		cin >> n >> m;
	}
