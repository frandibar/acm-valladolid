...
public:
	...
	// otros
	int distance(string fromVert, string toVert);
...
///////////////////////////////////////////////////////////////////////////
int Grafo::distance(string fromVert, string toVert)
// returns the minimum number of edges between fromVert and toVert
// uses BFS
{
	// check if both vertices exist
	int nfromVert, ntoVert;
	if (existeVertice(fromVert))
		nfromVert = m_mapVertices[fromVert];
	else
		return INFINITO;
	
	if (existeVertice(toVert))
		ntoVert = m_mapVertices[toVert];
	else
		return INFINITO;
	...
	queue<int> q;
	q.push(nfromVert);
	discovered[nfromVert] = true;
	int v = q.front();
	int distance = 0;
	while (!q.empty() && (v != ntoVert)) {
		distance++;
		q.pop();
		// recorrer la lista de aristas del vertice v
		list<Arista>::iterator LI;		// const
...
int main(int argc, char *argv[])
{
	...
	int cases;
	cin >> cases;
	for (int k = 1; k <= cases && !cin.eof(); k++) {
		// READ INPUT AND PROCESS
		Grafo g;		// non-directed graph
//		g.agregarVertice(ERDOS);
		...
		for (int i = 0; i < npapers; i++) {
			...
			// process paper
			// I'm assuming no paper has a ":" in it's name
			int to = line.find(":");
			while (to != string::npos) {
				int from = line.substr(0, to - 1).find_last_of(",");
				from = line.substr(0, from - 1).find_last_of(",");
				string name;
				if (from == string::npos)
					name = line.substr(0, to);
				else
					name = line.substr(from + 2, to - from - 2);
				g.agregarVertice(name);
				vecAuthors.push_back(name);				
				to = from;
			}
			// add edges between authors in vecAuthors
			for (int i = 0; i < (int)vecAuthors.size(); i++)
				for (int j = i + 1; j < (int)vecAuthors.size(); j++)
					g.agregarArista(vecAuthors[i], vecAuthors[j]);
		}

		// add to graph names from the list of names, and to vecNames
		// this is just in case names without paper appear (graph)
		for (int i = 0; i < nnames; i++) {
			getline(cin, line);
			vecNames.push_back(line);
//			g.agregarVertice(line);		// only if it doesn't exist
		}

#ifdef DEBUG
		cout << "START DEBUG ********************************************************************" << endl;
		cout << "aristas: " << g.cantAristas() << endl;
		cout << "vertices: " << g.cantVertices() << endl;
		g.mostrar();
		cout << "END DEBUG **********************************************************************" << endl;
#endif
		...
		for (int i = 0; i < nnames; i++) {
			int dist = g.distance(vecNames[i], ERDOS);
			cout << vecNames[i] << " ";
			if (dist == INFINITO)
				cout << "infinity" << endl;
			else
