...
// returns the sum of each number in vec_nums
{
	string res = "";
	// calculate result
	int digits = (int)vec_nums[0].length();
	int digit_sum = 0;
	int carry = 0;
	for (int i = digits - 1; i >= 0; i--) {
		digit_sum = add_digits(vec_nums, i) + carry;
		carry = digit_sum / 10;
		digit_sum %= 10;
	...
	int pos = (int)res.find_first_not_of("0");
...
// returns result with sx * cy (sx has n digits, y has only 1 digit)
{
	result = "";
	int y, carry = 0;
	y = cy - '0';
...
// shows multiplication of x by y
{
	...
	if (n1 == 0) {
		...
		int digits = std::max((int)x.length(), (int)y.length());
	...
	int ops = (int)y.length() - 1;
	for (int i = ops; i >= 0; i--) {
		char c = y[i];
		// do vec_partial[ops - i] = x * c;
		multiply_by_char(x, c, vec_partial[ops - i]);
		int len = (int)vec_partial[ops - i].length() + ops - i;
		if (len > max_len) 
			max_len = len;
	}
	...
	if (y.length() > 1) {
		...
		for (int i = 0; i < (int)y.length(); i++) {
			...
			if (vec_partial[i].find_first_not_of("0") != string::npos) {
				string partial = left_offset + vec_partial[i] + right_offset;
				vec_sums.push_back(partial);
				sums++;
			}
			right_offset += " ";
		}
	...
	if (sums > 1) {
		for (int i = 0; i < (int)vec_sums.size(); i++)
			cout << vec_sums[i] << endl;
		// show 2nd separator
